...
2. **classTime** 为每节课的**开始上课**时间，以元组形保存。如果 `class_time` 模块存有对应学校时间表导入即可，例如重庆文理学院即可使用 `class_time.cqwu` 导入预存有时间表，若无则在 `class_time.py` 中按照对应格式创建学校对应时间表导入即可，创建格式为：预留第 [0] 项为 None 是为了让后续读取第 1 节课时可以按照常规思路取第 [1] 项，而非第 [0] 项。请依次填写每节课的 24 小时制上课时间：例如 8:00 上课，则录入 `(8, 0)`；下午 7:50 上课，则录入 `(19, 50)`。
